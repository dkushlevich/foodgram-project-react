...
class TestRecipesAPI:
    ...
    def test_recipe_create_with_invalid_data(
            self, ingredient_1,
            ingredient_2, tag_1, user_client
    ):
    ...
    def test_recipe_create_with_valid_data(
        ...
        image_response = user_client.get(response.json()['image'])
        assert image_response.status_code == HTTPStatus.OK, (
            f'Проверьте, что в ответе на POST-запрос к `{self.recipes_url}` '
            'содержится валидная ссылка на картинку рецепта'
        )

    def test_acces_not_authenticated_recipes_delete(self, client, recipe_1):
    ...
    def test_delete_recipe_by_author(self, user_client, recipe_1):
    ...
    def test_delete_recipe_not_by_author(self, another_user_client, recipe_1):
    ...
    def test_delete_recipe_by_admin(self, admin_user_client, recipe_1):
    ...
    def test_acces_not_authenticated_recipes_patch(self, client, recipe_1):
    ...
    def test_patch_recipes_with_invalid_data(
            self, user_client, recipe_1, ingredient_1
    ):
